 ...
 ***********************************************************************************************************************
 */
void os_task_switch_fn(void)
{
    #ifdef OS_USING_OVERFLOW_CHECK
    task_switch_stack_check(g_os_current_task, g_os_next_task);
    #endif
...
#ifdef OS_MEM_ALGORITHM_MINIMIZATION
    stack_begin = OS_KERNEL_MALLOC(stack_size);
#else
...
os_err_t os_task_destroy(os_task_t *task)
{
    ...
    OS_ASSERT((task->state & OS_TASK_STATE_CLOSE) == 0U);
    OS_ASSERT(OS_FALSE == os_is_irq_active());
    OS_ASSERT(OS_FALSE == os_is_irq_disabled());
    OS_ASSERT(OS_FALSE == os_is_schedule_locked());
