...
void k_readyq_put(struct os_task *task)
{
    ...
    if ((gs_os_high_task == OS_NULL)
        || (priority < gs_os_high_task->current_priority))
    {
    	gs_os_high_task = task;
    }
...
void k_readyq_remove(struct os_task *task)
{
    ...
    if (os_list_empty(task_list_head))
    {
        ...
        if (task == gs_os_high_task)
        {
            highest_priority = _k_readyq_highest();
            gs_os_high_task  = os_list_entry(gs_os_readyq.priority_list_array[highest_priority].next, os_task_t, task_node);
        }
    }
    else
    {
...
void k_readyq_move_tail(struct os_task *task)
{
    ...
    if (task == gs_os_high_task)
    {
        gs_os_high_task = os_list_entry(gs_os_readyq.priority_list_array[priority].next, os_task_t, task_node);
    }
...
void k_start(void)
{
    g_os_next_task = gs_os_high_task;
 ...
 ***********************************************************************************************************************
 */ 
void k_kernel_exit_sched(void)
{
    /*
     * 1. Kernel is not initialized, must call k_start()
     * 2. Schedule is locked
     * 3. Next task is current task
     */
    if ((OS_NULL == g_os_current_task) || (0 != gs_os_sched_lock_cnt))
    {
        OS_KERNEL_EXIT();
    }
    else
...
void os_schedule_unlock(void)
{
    ...
    OS_ASSERT_EX(gs_os_sched_lock_cnt >= 0, "Task(%s) use schedule lock incorrectly.", g_os_current_task->name);

    if (0 == gs_os_sched_lock_cnt)
    {
        OS_KERNEL_ENTER();
