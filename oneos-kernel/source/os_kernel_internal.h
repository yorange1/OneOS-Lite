...
#define OS_KERNEL_VERSION                           "Kernel-V2.0.1-20210118"
...
extern void      k_tickq_check(void);
...
extern void      k_kernel_exit_sched(void);
...
extern void      k_show_blocked_task_old(os_list_node_t *list);
...
#define OS_KERNEL_ENTER()                                   \
    do                                                      \
    {                                                       \
        g_os_kernel_lock.irq_save = os_irq_lock();          \
        k_kernel_enter_check(&g_os_kernel_lock);            \
    } while (0)

#define OS_KERNEL_EXIT()                                    \
    do                                                      \
    {                                                       \
        k_kernel_exit_check(&g_os_kernel_lock);             \
        os_irq_unlock(g_os_kernel_lock.irq_save);           \
    } while (0)

#define OS_KERNEL_LOCK_REF_DEC()                            \
    do                                                      \
    {                                                       \
        g_os_kernel_lock.ref_cnt--;                         \
    } while (0) 
...
#define OS_KERNEL_EXIT_SCHED()  k_kernel_exit_sched()

extern struct kernel_lock g_os_kernel_lock;

extern void k_kernel_enter_check(struct kernel_lock *lock);
extern void k_kernel_exit_check(struct kernel_lock *lock);
