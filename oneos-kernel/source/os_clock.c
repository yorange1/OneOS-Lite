...
#define WAKE_UP_TASK_FROM_TICK_Q_BUCKET           (0)
#define WAKE_UP_TASK_FROM_ALL_TICK_Q_BUCKET       (1)
...
static os_uint32_t _k_wake_up_sleep_task(os_uint32_t wake_type)
{
    os_uint32_t need_sched = OS_FALSE;
    os_int32_t      bidx   = 0;
    if (WAKE_UP_TASK_FROM_TICK_Q_BUCKET == wake_type)
    {
        /* Process tick queue */
        bidx        = gs_os_tick & (TICK_Q_BUCKETS - 1);
        need_sched = _k_task_is_need_schedule(bidx);
    }
    else if(WAKE_UP_TASK_FROM_ALL_TICK_Q_BUCKET == wake_type)
    {
...
void os_tick_increase(void)
{
    ...
    gs_os_tick++;
    is_sched = _k_wake_up_sleep_task(WAKE_UP_TASK_FROM_TICK_Q_BUCKET);
    /* Process time slice */
...
OS_INLINE void _k_tick_update(os_tick_t sleep_ticks)
{
...
#if defined(OS_USING_TIMER) && defined(OS_TIMER_SORT)
...
void os_tickless_update(os_tick_t sleep_ticks)
{
    ...
    is_sched = k_timer_need_handle();
#endif

    _k_tick_update(sleep_ticks);
    
    is_sched |= _k_wake_up_sleep_task(WAKE_UP_TASK_FROM_ALL_TICK_Q_BUCKET);
