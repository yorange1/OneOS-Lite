...
static void _k_timer_task_entry(void *parameter)
{
    ...
    while (OS_TRUE)
    {
        ...
        else
        {
             ...
             * 1.Move the current active list pointed by gs_os_timer_list_current to expire list.
             * 2.Move gs_os_timer_list_current forward one step.
             * 3.Handle timers of expire list one by one.
             * 4.If the timer does not expire or timer is periodic,add the timer to restart list.
             * 5.Call timeout function of expired timer.
             * 6.Add the timer of restart list to active list.
             */
             ...
             *   -----      -----          -----      -----      -----         -----     -----      -----
             */
            os_list_add(&gs_os_timer_list_current->active_node.active_list, &expired_timer_head.active_list);
            os_list_del(&gs_os_timer_list_current->active_node.active_list);

            _k_timer_update_handle_flag((os_uint32_t)(gs_os_timer_list_current - gs_os_timer_list_start));
...
os_bool_t k_timer_need_handle(void)
{
     ...
     * Otherwise just move gs_os_timer_list_current forward one step.
     */
...
#endif
        {
            /* List is not empty,set flag. */
            gs_os_timer_need_handle = OS_TRUE;
        }
    }
    else
    {
        /* List is expty, just move gs_os_timer_list_current forward one step. */
        if (++gs_os_timer_list_current > gs_os_timer_list_end)
        {
            gs_os_timer_list_current = gs_os_timer_list_start;
        }
