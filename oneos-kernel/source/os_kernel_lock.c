...
void k_kernel_enter_check(struct kernel_lock *lock)
{
    ...
    if (OS_NULL != g_os_current_task)
    {
        ...
        {
            OS_ASSERT_EX(0 == lock->ref_cnt, "Kernel enter check failed, task(%s), irq(%u), ref_cnt(%d)",
...
void k_kernel_exit_check(struct kernel_lock *lock)
{
    ...
    if (OS_NULL != g_os_current_task)
    {
        ...
        {
            OS_ASSERT_EX(1 == lock->ref_cnt, "Kernel exit check failed, task(%s), irq(%u), ref_cnt(%d)",
                         g_os_current_task->name,
                         os_irq_num(),
                         lock->ref_cnt);
