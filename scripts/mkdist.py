...
def do_copy_file(src, dst):
    ...
    if not os.path.exists(path):
        os.makedirs(path_handle(path))

    shutil.copy2(path_handle(src), path_handle(dst))
...
def do_copy_folder(src_dir, dst_dir, ignore=None):
    ...
    try:
        if os.path.exists(dst_dir):
            shutil.rmtree(path_handle(dst_dir))
    except:
        print('Deletes folder: %s failed.' % dst_dir)
        return
    src_dir = os.path.abspath(src_dir)
    dst_dir = os.path.abspath(dst_dir)
    shutil.copytree(path_handle(src_dir), path_handle(dst_dir), ignore=ignore)
...
def zip_dist(dist_dir, dist_name):
    ...
    for parent, dirnames, filenames in os.walk(dist_dir):
        for filename in filenames:
            ...
            zip.write(path_handle(pathfile), arcname)
