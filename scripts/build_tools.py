...
if os.getenv("RUN_CLIENT") == "ONEOS_STUDIO" and os.path.exists("osconfig.json"):
    print("use osconfig.json to update osconfig.py")
    import json
    with open("osconfig.json", "r") as f:
        config_json = json.loads(f.read())
        cross_tool = os.getenv('OS_CC', "gcc")
        sys.path.append(os.getcwd())
        import osconfig
        for k,v in config_json[cross_tool].items():
            setattr(osconfig, k, v)
...
def SetupCompile(env, root_directory, has_libcpu=False, remove_components=[], SDK_LIB=None, SDK_DRIVER=None):
    ...
    AddOption('--update-head',
                dest = 'config-file',
                type = 'string',
                help = 'generate oneos_config.h from config file.')
    AddOption('--verbose',
                dest = 'verbose',
                action = 'store_true',
                default = False,
    ...
    AddOption('--studio',
                ...
                action = 'store_true',
                default = False,
                help = 'make code base for OneOS Studio IDE')
    ...
    if GetOption('single-compile')==False and GetOption('num_jobs')==1:
        ...
        print("Default using -j%d to compile, please use --single-compile option to specify single thread compiling."%cpu_count())
