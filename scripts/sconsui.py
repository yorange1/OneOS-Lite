...
class SconsUI():
    ...
    def read_setting(self):
        ...
        setting_path = os.path.join(home, '.oneos_scons')
        if os.path.exists(setting_path):
            setting = open(os.path.join(home, '.oneos_scons'))
            for line in setting:
                line = line.replace('\n', '')
                line = line.replace('\r', '')
                if line.find('=') != -1:
        ...
        # set  CMCC IOT Root Directory according environ
        if 'OS_ROOT' in os.environ:
            self.OSRoot.set_path(os.environ['OS_ROOT'])
        
        if self.OSRoot.get_path() == '':
            os_root = ''
            # detect CMCC IOT directory 
            if os.path.exists(os.path.join('..', 'include', 'os_kernel.h')):
                os_root = os.path.join('..')
            elif os.path.exists(os.path.join('..', '..', 'include', 'os_kernel.h')):
                os_root = os.path.join('..', '..')
            if os_root:
                self.OSRoot.set_path(os.path.abspath(os_root))
    ...
    def save_setting(self):
        ...
        setting = open(os.path.join(home, '.oneos_scons'), 'w+')
        # current comiler 
        ...
        setting.close()
        tkMessageBox.showinfo("CMCC IOT SCons UI",
    ...
    def setup_setting_ui(self, frame):
        row = 0
        label = ttk.Label (frame, relief = 'flat', text='CMCC IOT Root Folder:')
        label.grid(row=row, column=0,ipadx=5, ipady=5, padx = 5)
    
        self.OSRoot = DirSelectBox(frame)
        self.OSRoot.grid(row=row, column=1, sticky=E+W)
        row = row + 1
    ...
    def prepare_build(self):
        ...
        os.environ['OS_CC'] = compiler
        os.environ['OS_EXEC_PATH'] = exec_path
