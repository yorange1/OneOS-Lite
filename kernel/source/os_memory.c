...
extern void k_small_mem_init(struct heap_mem *h_mem, void *start_addr, os_size_t size);
#endif
...
{
#ifdef OBJECT_INIT_REPEAT_CHECK
    os_list_node_t *pos;
...
os_err_t os_memheap_add(os_memheap_t *heap, void *start_addr, os_size_t size, enum os_mem_alg alg)
{
    ...
    ret = OS_EOK;
    start = OS_ALIGN_UP((os_size_t)start_addr, 4);
    OS_ASSERT((os_size_t)start_addr + size > start);
    size  = OS_ALIGN_DOWN((os_size_t)start_addr + size - start, 4);
    ...
    if (size > sizeof(struct heap_mem))
    {
        ...
        os_list_for_each(pos, &gs_os_heap_resource_list_head)
        {
            ...
            while(h_mem_temp)
            {
                ...
                h_mem_temp_end = (os_size_t)h_mem_temp->header + h_mem_temp->mem_total;
                OS_ASSERT(h_mem_temp_start < h_mem_temp_end);

                if (((h_mem_temp_start > start) && (h_mem_temp_start < (start + size)))
                    || ((h_mem_temp_end > start) && (h_mem_temp_end < (start + size))))
                {
    ...
    if (OS_EOK == ret)
    {
        ...
        for (i = 0; i < sizeof(alg_init_table)/ sizeof(struct alg_init_func); i++)
        {
            ...
            {
                alg_init_table[i].mem_init(h_mem_new, (void *)start, size);
                break;
            }
        }
