...
static os_err_t clm920rv3_at_init(mo_object_t *self)
{
    ...
    if (OS_EOK != at_parser_exec_cmd(parser, &resp, "ATE0"))
        goto __exit;
    if (OS_EOK != at_parser_exec_cmd(parser, &resp, "AT+CREG=0"))
        goto __exit;
    if (OS_EOK != at_parser_exec_cmd(parser, &resp, "AT+CEREG=0"))
        goto __exit;
...
int clm920rv3_auto_create(void)
{
    ...
    INFO("Auto create %s module object with [%s]:[%d]bps",
         CLM920RV3_NAME,
         CLM920RV3_DEVICE_NAME,
         CLM920RV3_DEVICE_RATE);
