...
static void bc28_urc_receiver(struct at_parser *parser, const char *data, os_size_t size)
{
    ...
    bc28_onenet_mq_msg_t handle_msg = {module->regist_cb, urc_data, size, bc28_nb_urc_handler_table[handler_ref].func};
...
static void bc28_urc_manager_task(void *parameter)
{
    ...
    while (1)
    {
        ...
        /* wait & pop */
        result = os_mq_recv(bc28_nb_mq, &handle_msg, sizeof(handle_msg), OS_WAIT_FOREVER, &recv_size);
        if (OS_EOK != result)
        {
...
os_err_t bc28_onenetnb_init(mo_bc28_t *module)
{
    ...
    bc28_nb_mq = os_mq_create(BC28_URC_MQ_NAME, sizeof(bc28_onenet_mq_msg_t), BC28_URC_MQ_MSGNUM_MAX);
...
void bc28_onenetnb_deinit(mo_bc28_t *module)
{
    ...
    {
        memset(&handle_msg, 0, sizeof(bc28_onenet_mq_msg_t));
        result = os_mq_recv(bc28_nb_mq, &handle_msg, sizeof(handle_msg), OS_NO_WAIT, &recv_size);
        if (OS_EOK != result)
            break;
        os_free((void *)handle_msg.data);
    }
