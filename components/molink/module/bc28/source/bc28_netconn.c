...
mo_netconn_t *bc28_netconn_create(mo_object_t *module, mo_netconn_type_t type)
{
    ...
    netconn->mq = os_mq_create(BC28_NETCONN_MQ_NAME, BC28_NETCONN_MQ_MSG_SIZE, BC28_NETCONN_MQ_MSG_MAX);
...
#ifdef BC28_USING_PING_OPS
os_err_t
bc28_ping_handler(mo_object_t *module, const char *host, os_uint16_t len, os_uint32_t timeout, struct ping_resp *resp)
{
    at_parser_t *parser = &module->parser;
    os_err_t result = OS_EOK;
    ...
    if ((len < BC28_MIN_PING_PKG_LEN) || (len > BC28_MAX_PING_PKG_LEN))
    {
        ERROR("BC28 ping: ping package len[%d] is out of range[%d, %d].",
              len,
              BC28_MIN_PING_PKG_LEN,
              BC28_MAX_PING_PKG_LEN);
        return OS_ERROR;
    }
    ...
    if ((timeout < BC28_MIN_PING_TIMEOUT) || (timeout > BC28_MAX_PING_TIMEOUT))
    {
        ERROR("BC28 ping: user set ping timeout value %ums is out of range[%dms, %dms].",
              timeout,
              BC28_MIN_PING_TIMEOUT,
              BC28_MAX_PING_TIMEOUT);
        return OS_ERROR;
    }
