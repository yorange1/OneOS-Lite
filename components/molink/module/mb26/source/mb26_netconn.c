...
#define DBG_EXT_TAG "mb26.netconn"
...
#ifndef MB26_DATA_QUEUE_SIZE
#define MB26_DATA_QUEUE_SIZE (5)
#endif
...
mo_netconn_t *mb26_netconn_create(mo_object_t *module, mo_netconn_type_t type)
{
    ...
    {
        LOG_EXT_E("Module %s parse netconn id failed, id[%d] error", module->name, conn_id);
        mb26_unlock(&mb26->netconn_lock);
        return OS_NULL;
    }

    result = os_data_queue_init(&netconn->data_queue, MB26_DATA_QUEUE_SIZE, 0, OS_NULL);
    if (result != OS_EOK)
    {
...
os_err_t mb26_netconn_connect(mo_object_t *module, mo_netconn_t *netconn, ip_addr_t addr, os_uint16_t port)
{
    ...
    if (result != OS_EOK)
    {
        LOG_EXT_E("Module %s connection[%d] connect to %s:%d fail", module->name, netconn->connect_id, remote_ip, port);
        return result;
    }
...
static os_size_t mb26_tcp_udp_send(at_parser_t *parser, mo_netconn_t *netconn, const char *data, os_size_t size)
{
    ...
    while (sent_size < size)
    {
        ...
        snprintf(send_cmd, sizeof(send_cmd), "AT+SKTSEND=%d,%d,", netconn->connect_id, (int)cur_pkt_size / 2);
...
os_size_t mb26_netconn_send(mo_object_t *module, mo_netconn_t *netconn, const char *data, os_size_t size)
{
    ...
    mb26_lock(&mb26->netconn_lock);
    sent_size = mb26_tcp_udp_send(parser, netconn, hexstr, strlen(hexstr));
    mb26_unlock(&mb26->netconn_lock);
...
static void urc_close_func(struct at_parser *parser, const char *data, os_size_t size)
{
    ...
    if (OS_NULL == netconn)
    {
        LOG_EXT_E("Module %s connect[%d] netconn is NULL", module->name, connect_id);
        return;
    }

    LOG_EXT_W("Module %s receive close urc data of connect %d, err_code: %d", module->name, connect_id, err_code);
