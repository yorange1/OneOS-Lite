...
static os_err_t mb26_at_init(mo_object_t *self)
{
    ...
    if (result != OS_EOK)
    {
        LOG_EXT_E("%s module failed to close echo", self->name);
        return OS_ERROR;
    }
...
{
    mo_mb26_t *module = (mo_mb26_t *)malloc(sizeof(mo_mb26_t));
    if (OS_NULL == module)
    {
...
int mb26_auto_create(void)
{
    ...
    if (OS_NULL == device)
    {
        LOG_EXT_E("Auto create failed, Can not find MB26 interface device %s!", MB26_DEVICE_NAME);
        return OS_ERROR;
    }
    ...
    if (OS_NULL == module)
    {
        LOG_EXT_E("Auto create failed, Can not create %s module object!", MB26_NAME);
        return OS_ERROR;
    }

    LOG_EXT_I("Auto create %s module object success!", MB26_NAME);
    return OS_EOK;
}
OS_CMPOENT_INIT(mb26_auto_create);
