...
#define DBG_EXT_TAG "mb26.general"
...
os_err_t mb26_get_firmware_version(mo_object_t *self, mo_firmware_version_t *version)
{
   at_parser_t *parser = &self->parser;
    ...
    LOG_EXT_D("Get %s module version resp line counts: %d", self->name, resp.line_counts);

    version->ver_info = calloc(resp.line_counts - 2, sizeof(char *));
