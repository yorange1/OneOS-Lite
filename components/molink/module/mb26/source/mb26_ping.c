...
os_err_t mb26_ping(mo_object_t *self, const char *host, os_uint16_t len, os_uint32_t timeout, struct ping_resp *resp)
{
    ...
    if (parser == OS_NULL)
    {
        LOG_EXT_E("MB26 ping: at parser is NULL.");
        return OS_ERROR;
    }
    ...
    LOG_EXT_D("MB26 ping: %s, len: %u, timeout: %ums", host, len, timeout);
    ...
    {
        LOG_EXT_E("Ping: AT cmd exec fail: AT+ECPING=\"%s\",1,%u,%u", host, len, timeout);
        result = OS_ERROR;
        goto __exit;
    }
    ...
    {
        LOG_EXT_D("MB26 ping: resp parse ip[%s], req_time[%u]", ipaddr, req_time);
        if (req_time >= timeout)
        {
            result = OS_ETIMEOUT;
        }
