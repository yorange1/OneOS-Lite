...
static os_size_t at_parser_vasprintf(at_parser_t *parser, const char *format, va_list args)
{
    ...
    len = vsnprintf(parser->at_cmd_buff, len + 1, format, args);
    ...
    while (sent_size < len)
    {
        sent_size += os_device_write_nonblock(parser->device, 0, parser->at_cmd_buff + sent_size, len - sent_size);
    }
...
static os_size_t at_parser_vasprintfln(at_parser_t *parser, const char *format, va_list args)
{
    ...
    os_device_write_nonblock(parser->device, 0, "\r\n", 2);
...
os_size_t at_parser_send(at_parser_t *parser, const char *buf, os_size_t size)
{
    ...
    {
        sent_size += os_device_write_nonblock(parser->device, 0, buf + sent_size, size - sent_size);
    }
...
os_err_t at_parser_init(at_parser_t *parser, const char *name, os_device_t *device, os_size_t recv_buff_len)
{
    ...
    os_snprintf(name_buffer, OS_NAME_MAX, "%s_rx", name);

    os_event_init(&(parser->rx_notice_evt), name_buffer);

    os_snprintf(name_buffer, OS_NAME_MAX, "%s_resp", name);

    os_sem_init(&(parser->resp_notice), name_buffer, 0, OS_SEM_MAX_VALUE);

    os_snprintf(name_buffer, OS_NAME_MAX, "%s_lock", name);
    ...
    os_snprintf(name_buffer, OS_NAME_MAX, "%s_parser", name);
