...
static os_err_t atest_run_a_testcase(atest_tc_entry_t *tc_entry, atest_ctrl_info_t *ctrl_info)
{
    ...
    do
    {
        ...
        if (OS_NULL != tc_entry->init)
        {
            ...
            if (OS_EOK != init_ret)
            {
                os_kprintf("[   TC   ] [ FAILED ] Testcase (%s) init failed\r\n", tc_entry->name);
        ...
        {
            os_kprintf("[   TC   ] [ FAILED ] Testcase (%s) has no execute function.\r\n", tc_entry->name);
            ret = OS_ERROR;
        }
        ...
        if (OS_NULL != tc_entry->cleanup)
        {
            ...
            {
                os_kprintf("[   TC   ] [ FAILED ] Testcase (%s) cleanup failed.\r\n", tc_entry->name);
...
static void atest_tc_do_run(void *arg)
{
    ...
    {  
        ...
        if (!found_entry)
        {
            os_kprintf("[ ATEST  ] [ ====== ] Not find testcase(%s).\r\n", ctrl_info->tc_name);
            break;
        }
    ...
    do
    {
        ...
        else
        {
            ...
            if (OS_NULL != task)
            {
                ...
                if (OS_EOK != ret)
                {
                    os_kprintf("Atest start task to execute testcase failed\r\n");
                }
            }
            else
            {
