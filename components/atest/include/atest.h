...
#define ATEST_TC_EXPORT(name, testcase, init, cleanup, priority)                                                       \
    OS_USED static const atest_tc_entry_t gs_atest##testcase OS_SECTION(                                               \
        "AtestTcTab") = {#name, init, testcase, cleanup, priority};
...
#define tp_assert_str_equal(a, b)                                                                                      \
    atest_assert_string((const char *)(a), (const char *)(b), OS_TRUE, __FILE__, __LINE__, __func__, "string not equal")

/* If @a not equal to @b, not assert, means passing. String type test. */
#define tp_assert_str_not_equal(a, b)                                                                                  \
    atest_assert_string((const char *)(a), (const char *)(b), OS_FALSE, __FILE__, __LINE__, __func__, "string equal")

/* If @a equal to @b, not assert, means passing. Buffer type test. */
#define tp_assert_buf_equal(a, b, size)                                                                                \
    atest_assert_buf((const os_uint8_t *)(a),                                                                          \
                     (const os_uint8_t *)(b),                                                                          \
                     (size),                                                                                           \
                     OS_TRUE,                                                                                          \
                     __FILE__,                                                                                         \
...
#define tp_assert_buf_not_equal(a, b, size)                                                                            \
    atest_assert_buf((const os_uint8_t *)(a),                                                                          \
                     (const os_uint8_t *)(b),                                                                          \
                     (size),                                                                                           \
                     OS_FALSE,                                                                                         \
...
#define tp_assert_in_range(value, min, max)                                                                            \
    __atest_assert(((value >= min) && (value <= max)), "(" #value ") not in range(" #min "," #max ")")

/* If @value is not in range of @min and @max, not assert, means passing. */
#define tp_assert_not_in_range(value, min, max)                                                                        \
    __atest_assert(!((value >= min) && (value <= max)), "(" #value ") in range(" #min "," #max ")")
...
extern void atest_assert(os_bool_t condition, const char *file, os_int32_t line, const char *func, const char *msg);
