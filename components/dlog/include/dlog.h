...
#if (DLOG_ERROR <= DLOG_COMPILE_LEVEL)
#define LOG_E(tag, fmt, ...) dlog_output(DLOG_ERROR, tag, OS_TRUE, fmt, ##__VA_ARGS__)
#else
#define LOG_E(tag, fmt, ...)
#endif

#if (DLOG_WARNING <= DLOG_COMPILE_LEVEL)
#define LOG_W(tag, fmt, ...) dlog_output(DLOG_WARNING, tag, OS_TRUE, fmt, ##__VA_ARGS__)
#else
#define LOG_W(tag, fmt, ...)
#endif

#if (DLOG_INFO <= DLOG_COMPILE_LEVEL)
#define LOG_I(tag, fmt, ...) dlog_output(DLOG_INFO, tag, OS_TRUE, fmt, ##__VA_ARGS__)
#else
#define LOG_I(tag, fmt, ...)
#endif

#if (DLOG_DEBUG <= DLOG_COMPILE_LEVEL)
#define LOG_D(tag, fmt, ...) dlog_output(DLOG_DEBUG, tag, OS_TRUE, fmt, ##__VA_ARGS__)
#else
#define LOG_D(tag, fmt, ...)
#endif
...
struct dlog_backend
{
    ...
    void (*output)(struct dlog_backend *backend, char *log, os_size_t len); /* Output log to backend */
