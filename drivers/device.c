...
os_err_t os_device_register(os_device_t *dev, const char *name)
{
    ...
    strncpy(device_name(dev), name, OS_DEVICE_NAME_MAX);
...
#if defined(OS_USING_DEVICE_BLOCK)
    os_sem_init(&dev->tx_sem, name, 1, OS_SEM_MAX_VALUE);
    os_sem_init(&dev->rx_sem, name, 0, OS_SEM_MAX_VALUE);
#endif
...
os_device_t *os_device_find(const char *name)
{
    ...
    {
        if (0 == strncmp(device_name(dev), name, OS_DEVICE_NAME_MAX))
        {
            os_sem_post(&dev_sem);
            return dev;
        }
...
os_err_t os_device_open(os_device_t *dev)
{
    ...
    if (dev->ref_count == 0)
    { 
        if(dev->state == OS_DEVICE_STATE_CLOSING)
        {
            result = OS_EBUSY;
            goto end;
        }
    ...
    dev->state = OS_DEVICE_STATE_OPEN;
    dev->ref_count++;
...
os_err_t os_device_close(os_device_t *dev)
{
    ...
    {
        result = dev->ops->deinit(dev);
    }

    if (result == OS_EBUSY)
    {
        dev->state = OS_DEVICE_STATE_CLOSING;
        result = OS_EOK;
    }
    else
    {
...
os_err_t os_device_control(os_device_t *dev, int cmd, void *arg)
{
    ...
    switch (cmd)
    {
        ...
        case OS_DEVICE_CTRL_SUSPEND:
            dev->state |= OS_DEVICE_STATE_SUSPEND;
            break;
...
os_err_t sh_list_device(os_int32_t argc, char **argv)
{
    ...
    os_kprintf("%-*.s         type         ref count\r\n", OS_DEVICE_NAME_MAX, "device");
    for (i = 0; i < OS_DEVICE_NAME_MAX; i++)
    {
        os_kprintf("-");
    }
    os_kprintf(" -------------------------------\r\n");
