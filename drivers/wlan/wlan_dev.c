...
#define WLAN_DEV_LOCK(_wlan)   (os_mutex_recursive_lock(&(_wlan)->lock, OS_WAIT_FOREVER))
...
os_err_t os_wlan_dev_init(struct os_wlan_device *device, os_wlan_mode_t mode)
{
    ...
    {
        LOG_E(DBG_TAG,"F:%s L:%d Parameter Wrongful device:0x%08x mode:%d", __FUNCTION__, __LINE__, device, mode);
        return OS_ERROR;
    }

    if (mode == OS_WLAN_AP && device->flags & OS_WLAN_FLAG_STA_ONLY)
    {
        LOG_E(DBG_TAG,"F:%s L:%d This wlan device can only be set to sta mode!", __FUNCTION__, __LINE__);
        return OS_ERROR;
    }
    else if (mode == OS_WLAN_STATION && device->flags & OS_WLAN_FLAG_AP_ONLY)
    {
        LOG_E(DBG_TAG,"F:%s L:%d This wlan device can only be set to ap mode!", __FUNCTION__, __LINE__);
        return OS_ERROR;
    }

    result = os_device_open(OS_DEVICE(device));
    if (result != OS_EOK)
    {
        LOG_E(DBG_TAG,"L:%d wlan init failed", __LINE__);
        return OS_ERROR;
    }
    result = os_device_control(OS_DEVICE(device), OS_WLAN_CMD_MODE, (void *)&mode);
    if (result != OS_EOK)
    {
        LOG_E(DBG_TAG,"L:%d wlan config mode failed", __LINE__);
        return OS_ERROR;
    }
    device->mode = mode;
    return result;
...
os_err_t os_wlan_dev_connect(struct os_wlan_device *device, struct os_wlan_info *info, const char *password, int password_len)
{
    ...
    {
        LOG_E(DBG_TAG,"L:%d password or ssid is too long", __LINE__);
        return OS_ERROR;
    }
...
os_err_t os_wlan_dev_scan(struct os_wlan_device *device, struct os_wlan_info *info)
{
    ...
    if (info != OS_NULL)
    {
        ...
        {
            LOG_E(DBG_TAG,"L:%d ssid is too long", __LINE__);
            return OS_EINVAL;
        }
...
static os_err_t _os_wlan_dev_init(os_device_t *dev)
{
    ...
    if (result == OS_EOK)
    {
        LOG_I(DBG_TAG,"wlan init success");
    }
    else
    {
        LOG_I(DBG_TAG,"wlan init failed");
    }
                              ...
                              void                         *user_data)
{
    ...
    {
        LOG_E(DBG_TAG,"F:%s L:%d parameter Wrongful", __FUNCTION__, __LINE__);
    ...
    wlan->flags = flag;
    err         = os_device_register(&wlan->device, name);
