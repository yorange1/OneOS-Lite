...
#define WLAN_CFG_LOCK()   (os_mutex_recursive_lock(&cfg_mutex, OS_WAIT_FOREVER))
...
void os_wlan_cfg_init(void)
{
    ...
    {
        cfg_cache = os_calloc(1, sizeof(struct os_wlan_cfg_des));
        if (cfg_cache != OS_NULL)
        {
...
int os_wlan_cfg_delete_index(int index)
{
    ...
    /* malloc new mem */
    cfg_info = os_calloc(1, sizeof(struct os_wlan_cfg_info) * (cfg_cache->num - 1));
    if (cfg_info == OS_NULL)
    {
        WLAN_CFG_UNLOCK();
        return -1;
