...
static void netif_is_ready(struct os_work *work, void *data)
{
    ...
    {
        LOG_W(DBG_TAG,"F:%s L:%d ip addr type not support", __FUNCTION__, __LINE__);
    }
#else
#if LWIP_IPV4
    ...
    *(ip_addr_t *)(&ip_addr[0]) = eth_dev->netif->ip_addr;
    ...
    os_schedule_unlock();
    LOG_I(DBG_TAG,"Got IP address : %s", str);
exit:
    level = os_irq_lock();
    os_irq_unlock(level);
}
...
static os_err_t os_wlan_lwip_protocol_recv(struct os_wlan_device *wlan, void *buff, int len)
{
...
#else
    {
        ...
        while (p == OS_NULL)
        {
            ...
            if (count >= 10)
            {
                LOG_W(DBG_TAG,"F:%s L:%d pbuf allocate fail!!!", __FUNCTION__, __LINE__);
                return OS_ENOMEM;
            }
        }
        /*copy data dat -> pbuf*/
...
static os_err_t os_wlan_lwip_protocol_send(os_device_t *device, struct pbuf *p)
{
...
#else
    {
        ...
        if (frame == OS_NULL)
        {
            LOG_E(DBG_TAG,"F:%s L:%d malloc out_buf fail\r\n", __FUNCTION__, __LINE__);
            return OS_ENOMEM;
        }
        /*copy pbuf -> data dat*/
...
static struct os_wlan_prot *os_wlan_lwip_protocol_register(struct os_wlan_prot *prot, struct os_wlan_device *wlan)
{
    ...
    if (id > 9)
    {
        LOG_E(DBG_TAG,"F:%s L:%d not find Empty name", __FUNCTION__, __LINE__, eth_name);
        return OS_NULL;
    }

    if (os_device_open((os_device_t *)wlan) != OS_EOK)
    {
        LOG_E(DBG_TAG,"F:%s L:%d open wlan failed", __FUNCTION__, __LINE__);
        return OS_NULL;
    }

    lwip_prot = os_calloc(1, sizeof(struct lwip_prot_des));
    if (lwip_prot == OS_NULL)
    {
        LOG_E(DBG_TAG,"F:%s L:%d malloc mem failed", __FUNCTION__, __LINE__);
    ...
    {
        LOG_E(DBG_TAG,"eth device init failed");
        os_device_close((os_device_t *)wlan);
        os_free(lwip_prot);
        return OS_NULL;
    }
    ...
    LOG_I(DBG_TAG,"eth device init ok name:%s", eth_name);
...
int os_wlan_lwip_init(void)
{
    ...
    {
        LOG_E(DBG_TAG,"F:%s L:%d protocol regisetr failed", __FUNCTION__, __LINE__);
        return -1;
    }
