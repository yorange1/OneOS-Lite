...
os_err_t os_wlan_workqueue_dowork(void (*func)(void *parameter), void *parameter)
{
    ...
    if (func == OS_NULL)
    {
        LOG_E(DBG_TAG,"F:%s L:%d func is null", __FUNCTION__, __LINE__);
        return OS_EINVAL;
    }

    if (wlan_workqueue == OS_NULL)
    {
        LOG_E(DBG_TAG,"F:%s L:%d not init wlan work queue", __FUNCTION__, __LINE__);
        return OS_ERROR;
    }

    wlan_work = os_calloc(1, sizeof(struct os_wlan_work));
    if (wlan_work == OS_NULL)
    {
        LOG_E(DBG_TAG,"F:%s L:%d create work failed", __FUNCTION__, __LINE__);
        return OS_ENOMEM;
    }
    wlan_work->fun       = func;
    ...
    if (err != OS_EOK)
    {
        LOG_E(DBG_TAG,"F:%s L:%d do work failed", __FUNCTION__, __LINE__);
        os_free(wlan_work);
        return err;
    }
    return err;
...
int os_wlan_workqueue_init(void)
{
    ...
    if (_init_flag == 0)
    {
        ...
        if (wlan_workqueue == OS_NULL)
        {
            LOG_E(DBG_TAG,"F:%s L:%d wlan work queue create failed", __FUNCTION__, __LINE__);
            return -1;
        }
        _init_flag = 1;
        return 0;
