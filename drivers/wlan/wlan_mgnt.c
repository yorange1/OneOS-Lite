...
#define OS_WLAN_LOG_I(...)        LOG_I(DBG_TAG,__VA_ARGS__)
#define OS_WLAN_LOG_W(_fmt, ...)  LOG_W(DBG_TAG,"F:%s L:%d "_fmt"", __FUNCTION__, __LINE__, ##__VA_ARGS__)
#define OS_WLAN_LOG_E(_fmt, ...)  LOG_E(DBG_TAG,"F:%s L:%d "_fmt"", __FUNCTION__, __LINE__, ##__VA_ARGS__)

#define STA_DEVICE() (_sta_mgnt.device)
#define AP_DEVICE()  (_ap_mgnt.device)

#define SRESULT_LOCK()   (os_mutex_recursive_lock(&scan_result_mutex, OS_WAIT_FOREVER))
#define SRESULT_UNLOCK() (os_mutex_recursive_unlock(&scan_result_mutex))

#define STAINFO_LOCK()   (os_mutex_recursive_lock(&sta_info_mutex, OS_WAIT_FOREVER))
#define STAINFO_UNLOCK() (os_mutex_recursive_unlock(&sta_info_mutex))

#define MGNT_LOCK()   (os_mutex_recursive_lock(&mgnt_mutex, OS_WAIT_FOREVER))
#define MGNT_UNLOCK() (os_mutex_recursive_unlock(&mgnt_mutex))

#define COMPLETE_LOCK()   (os_mutex_recursive_lock(&complete_mutex, OS_WAIT_FOREVER))
...
static os_err_t os_wlan_send_to_thread(os_wlan_event_t event, void *buff, int len)
{
    ...
    /* Event packing */
    msg = os_calloc(1, sizeof(struct os_wlan_msg) + len);
    if (msg == OS_NULL)
    {
...
static os_err_t os_wlan_scan_result_cache(struct os_wlan_info *info, int timeout)
{
    ...
    /* malloc memory */
    ptable = os_calloc(1, sizeof(struct os_wlan_info) * (scan_result.num + 1));
    if (ptable == OS_NULL)
    {
        os_mutex_recursive_unlock(&scan_result_mutex);
...
static os_err_t os_wlan_sta_info_add(struct os_wlan_info *info, int timeout)
{
    ...
    {
        /* malloc memory */
        sta_list = os_calloc(1, sizeof(struct os_wlan_sta_list));
        if (sta_list == OS_NULL)
        {
...
static void os_wlan_cyclic_check(void *parameter)
{
    ...
    struct os_work      *work = os_calloc(1, sizeof(struct os_work));
    os_base_t            level;
                                   ...
                                   void                  *parameter)
{
    ...
    case OS_WLAN_DEV_EVT_AP_STOP:
    {
        ...
        err        = (os_err_t *)os_wlan_sta_info_del_all(OS_WAIT_FOREVER);
        if (err != OS_NULL)
        {
    ...
    case OS_WLAN_DEV_EVT_AP_ASSOCIATED:
    {
        ...
        if (user_buff.len != sizeof(struct os_wlan_info))
            break;
        err = (os_err_t *)os_wlan_sta_info_add(user_buff.data, OS_WAIT_FOREVER);
        if (err != OS_EOK)
        {
    ...
    case OS_WLAN_DEV_EVT_AP_DISASSOCIATED:
    {
        ...
        if (user_buff.len != sizeof(struct os_wlan_info))
            break;
        err = (os_err_t *)os_wlan_sta_info_del(user_buff.data, OS_WAIT_FOREVER);
        if (err != OS_EOK)
        {
...
static struct os_wlan_complete_des *os_wlan_complete_create(const char *name)
{
    ...
    complete = os_calloc(1, sizeof(struct os_wlan_complete_des));
    if (complete == OS_NULL)
    {
