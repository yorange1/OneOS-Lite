...
static struct os_wlan_device *os_wlan_prot_find_by_name(const char *name)
{
    ...
    if (device == OS_NULL)
    {
        LOG_E(DBG_TAG,"F:%s L:%d not find wlan dev!! name:%s", __FUNCTION__, __LINE__, name);
        return OS_NULL;
    }
...
os_err_t os_wlan_prot_attach_dev(struct os_wlan_device *wlan, const char *prot_name)
{
    ...
    if (wlan == OS_NULL)
    {
        LOG_E(DBG_TAG,"F:%s L:%d wlan is null", __FUNCTION__, __LINE__);
        return OS_ERROR;
    }
    ...
    {
        LOG_E(DBG_TAG,"F:%s L:%d not find wlan protocol", __FUNCTION__, __LINE__);
        return OS_ERROR;
    }
...
os_err_t os_wlan_prot_regisetr(struct os_wlan_prot *prot)
{
    ...
    if (i >= OS_WLAN_PROT_MAX)
    {
        LOG_E(DBG_TAG,"F:%s L:%d Space full", __FUNCTION__, __LINE__);
        return OS_ERROR;
    }
