...
static os_size_t stmpe811_read_point(struct os_touch_device *touch, void *buf, os_size_t read_num)
{
    uint16_t input_x  = 0;
    uint16_t input_y  = 0;
    uint16_t input_w  = 0;
    ...
    input_x = stmpe811->x_range - input_x;
    if (input_x > stmpe811->x_range)
        input_x = 0;
#endif

#ifdef OS_TOUCH_Y_SWAP
    input_y = stmpe811->y_range - input_y;
    if (input_y > stmpe811->y_range)
        input_y = 0;
#endif

    if (touch->info.range_x != stmpe811->x_range)
        input_x = input_x * touch->info.range_x / stmpe811->x_range;

    if (touch->info.range_y != stmpe811->y_range)
        input_y = input_y * touch->info.range_y / stmpe811->y_range;
