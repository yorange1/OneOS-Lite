...
#if defined(OS_USING_TICKLESS_LPMGR) || defined(OS_USING_SIMPLE_LPM)
static void os_clocksource_update_ns_cs(os_clocksource_t *cs,os_uint64_t nsec)
{
    cs->last_update_nsec += nsec;
}

void os_clocksource_update_ns(os_uint64_t nsec)
{
     os_base_t level;
     os_clocksource_t *cs;
     ...
     os_list_for_each_entry(cs, &gs_clocksource_list, os_clocksource_t, list)
     {
         os_clocksource_update_ns_cs(cs,nsec);
     }
