...
static os_uint32_t spixfer(struct os_spi_device *device, struct os_spi_message *message)
{
    ...
    struct gd32_spi *spi_drv = os_container_of(device->bus, struct gd32_spi, spi_bus);
    struct gd32_hw_spi_cs *cs_pin = device->parent.user_data;


    if (message->cs_take)
    {
        gpio_bit_reset(cs_pin->gpio_periph, cs_pin->pin);
        LOG_EXT_D("spi take cs\n");
    }
    ...
    {
        gpio_bit_set(cs_pin->gpio_periph, cs_pin->pin);
        LOG_EXT_D("spi release cs\n");
    }
...
os_err_t os_hw_spi_device_attach(const char *bus_name, const char *device_name, os_base_t cs_pin)
{
    ...
    struct gd32_hw_spi_cs *hw_cs_pin;
    hw_cs_pin = (struct gd32_hw_spi_cs *)os_calloc(1, sizeof(struct gd32_hw_spi_cs));
    OS_ASSERT(hw_cs_pin != OS_NULL);
    index = get_pin(cs_pin);
    if (index == OS_NULL)
    {
        return OS_ERROR;
    }
    ...
    hw_cs_pin->gpio_periph = index->gpio_periph;
    hw_cs_pin->pin = index->pin;
    rcu_periph_clock_enable((rcu_periph_enum)(hw_cs_pin->gpio_periph));
    gpio_mode_set(hw_cs_pin->gpio_periph, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, hw_cs_pin->pin);
    gpio_output_options_set(hw_cs_pin->gpio_periph, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, hw_cs_pin->pin);
    gpio_bit_set(hw_cs_pin->gpio_periph, hw_cs_pin->pin);
