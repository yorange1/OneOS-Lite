...
static os_uint32_t spixfer(struct os_spi_device *device, struct os_spi_message *message)
{
    ...
    if (message->cs_release)
    {
        ...
        LOG_D(DRV_EXT_TAG, "spi release cs\n");
    }
...
os_err_t os_hw_spi_device_attach(const char *bus_name, const char *device_name, os_base_t cs_pin)
{
    ...
    gpio_init(hw_cs_pin->gpio_periph, GPIO_MODE_OUT_PP, GPIO_OSPEED_50MHZ, hw_cs_pin->pin);
    ...
    if (result != OS_EOK)
    {
        LOG_E(DRV_EXT_TAG, "%s attach to %s faild, %d\n", device_name, bus_name, result);
    }

    OS_ASSERT(result == OS_EOK);

    LOG_D(DRV_EXT_TAG, "%s attach to %s done", device_name, bus_name);
