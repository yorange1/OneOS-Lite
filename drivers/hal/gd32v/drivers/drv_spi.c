...
os_err_t os_hw_spi_device_attach(const char *bus_name, const char *device_name, os_base_t cs_pin)
{
    ...
    hw_cs_pin = (struct gd32_hw_spi_cs *)os_malloc(sizeof(struct gd32_hw_spi_cs));
    ...
    hw_cs_pin->gpio_periph = index->gpio_port;
    ...
    if (result != OS_EOK)
    {
        LOG_E(DRV_TAG,"%s attach to %s faild, %d\n", device_name, bus_name, result);
    }

    OS_ASSERT(result == OS_EOK);

    LOG_D(DRV_TAG,"%s attach to %s done", device_name, bus_name);
