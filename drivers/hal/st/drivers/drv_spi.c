...
static void spi_dummy_xfer(struct os_spi_device *dev)
{
    uint8_t dummy_cmd[1] = {0x00};
    uint8_t dummy_rcv[1];
    
    struct stm32_spi  *spi_drv    = os_container_of(dev->bus, struct stm32_spi, spi_bus);
    SPI_HandleTypeDef *spi_handle = spi_drv->hspi;   
    struct stm32_hw_spi_cs *__cs_pin = dev->parent.user_data;    
    
    HAL_GPIO_WritePin(__cs_pin->GPIOx, __cs_pin->GPIO_Pin, GPIO_PIN_RESET);    
    HAL_SPI_TransmitReceive(spi_handle, (uint8_t *)dummy_cmd, (uint8_t *)dummy_rcv, 1, 1000);    
    HAL_GPIO_WritePin(__cs_pin->GPIOx,  __cs_pin->GPIO_Pin, GPIO_PIN_SET); 
}
os_err_t os_hw_spi_device_attach(const char *bus_name, const char *device_name, os_base_t cs_pin)
{
    ...
    spi_device = (struct os_spi_device *)os_calloc(1, sizeof(struct os_spi_device));
    OS_ASSERT(spi_device != OS_NULL);
    __cs_pin = (struct stm32_hw_spi_cs *)os_calloc(1, sizeof(struct stm32_hw_spi_cs));
    ...
    /*
     *Because there will be a level jump during the initialization of the soft cs pin, a certain 
     *probability will cause spi bus errors, and the wrong data is read for the first time. 
     *Therefore, the dummy command operation is added during the initialization process to solve
     *the problem
    */
    spi_dummy_xfer(spi_device);
