...
void HAL_SAI_RxCpltCallback(SAI_HandleTypeDef *hsai)
{
    struct stm32_sai *sai_dev;   
    os_uint16_t i;
    os_uint8_t temp = sai_dev->fifo.fifo_wr_pos; 
    ...
    os_list_for_each_entry(sai_dev, &stm32_sai_list, struct stm32_sai, list)
    {
        ...
        {                 
            ...
            sai_dev->fifo.fifo_wr_pos++;

            if(sai_dev->fifo.fifo_wr_pos >= OS_AUDIO_RECORD_FIFO_COUNT)
                sai_dev->fifo.fifo_wr_pos = 0; 

            if(sai_dev->fifo.fifo_rd_pos == sai_dev->fifo.fifo_wr_pos)
            {
                sai_dev->fifo.fifo_wr_pos = temp; 
            }
            else
            {
                for(i = 0; i < OS_AUDIO_RECORD_FIFO_SIZE; i++)
                {
                    sai_dev->fifo.rx_fifo[sai_dev->fifo.fifo_wr_pos][i] = sai_dma_recv[i + OS_AUDIO_RECORD_FIFO_SIZE];
                }
                os_sem_post(&sai_dev->sem);
            }
    ...
    os_list_for_each_entry(sai_dev, &stm32_sai_list, struct stm32_sai, list)
    {
        if (hsai == sai_dev->hsai)
        {
            ...
            else
            {
                for(i = 0; i < OS_AUDIO_RECORD_FIFO_SIZE; i++)
                {
                    sai_dev->fifo.rx_fifo[sai_dev->fifo.fifo_wr_pos][i] = sai_dma_recv[i];
                }
                os_sem_post(&sai_dev->sem);
            }
