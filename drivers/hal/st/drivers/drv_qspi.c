                                      ...
                                      void (*exit_qspi_mode)())
{
    ...
    cs_pin = (struct stm32_hw_spi_cs *)os_calloc(1, sizeof(struct stm32_hw_spi_cs));
    if (qspi_device == OS_NULL)
    {
        LOG_E(DBG_TAG,"no memory, qspi bus attach device failed!");
        result = OS_ENOMEM;
    ...
    result = os_spi_bus_attach_device(&qspi_device->parent, device_name, bus_name, (void *)cs_pin);
