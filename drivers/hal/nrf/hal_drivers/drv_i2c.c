...
void nrf5_i2c_read(nrfx_twim_t const * p_instance, unsigned char dev_addr, unsigned char *reg_addr, unsigned char *data, uint16_t len)
{
    ret_code_t err_code;

    err_code = nrf5_i2c_tx(p_instance,dev_addr,reg_addr,1,true);
    OS_ASSERT(err_code == 0);
    err_code = nrf5_i2c_rx(p_instance, dev_addr, data, len);
    OS_ASSERT(err_code == 0);
}
                                    ...
                                    os_uint32_t num)
{
    ...
    {
        nrf5_i2c_read(&nrf5_i2c->i2c_info->hi2c
...
static int nrf5_i2c_probe(const os_driver_info_t *drv, const os_device_info_t *dev)
{
    ...
    //os_sem_init(&nrf5_i2c->sem, name, 0, OS_IPC_FLAG_FIFO);
