...
void timer_irq_callback(nrf_timer_event_t event_type, void* p_context)
{
    ...
    {
        if(&timer->info->timer_periph == (nrfx_timer_t *)p_context && event_type == NRF_TIMER_EVENT_COMPARE0){
...
static void nrf5_timer_stop(os_clockevent_t *ce)
{
    ...
    nrfx_timer_disable(&timer->info->timer_periph);
}
...
OS_DRIVER_DEFINE(nrf5_timer_driver, PREV, OS_INIT_SUBLEVEL_MIDDLE);
