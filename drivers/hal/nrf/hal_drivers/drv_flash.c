...
static uint32_t GetPage(uint32_t Addr)
{
    ...
    {
        page = (Addr - NRF52_FLASH_START_ADDRESS + BLE_SOFTDEVICE_SIZE) / NRF52_FLASH_PAGE_SIZE;
    }
    else
    {
        return 0xffffffff;
...
int nrf52_flash_erase(uint32_t addr, size_t size)
{
    ...
    {
        result = nrfx_nvmc_page_erase((FirstPage + i) * NRF52_FLASH_PAGE_SIZE);
    ...
    LOG_D(DBG_TAG,"erase done: addr (0x%p), size %d", (void *)addr, NbOfPages * NRF52_FLASH_PAGE_SIZE);
    return size;
}
...
{
    int count = nrf52_flash_read(NRF52_FLASH_START_ADDRESS + page_addr * NRF52_FLASH_BYTE_ALIGN_SIZE, buff, page_nr * NRF52_FLASH_BYTE_ALIGN_SIZE);

    return (count == page_nr * NRF52_FLASH_BYTE_ALIGN_SIZE) ? 0 : -1;
}

static int fal_flash_write(fal_flash_t *flash, os_uint32_t page_addr, const os_uint8_t *buff, os_uint32_t page_nr)
{
    int count = nrf52_flash_write(NRF52_FLASH_START_ADDRESS + page_addr * NRF52_FLASH_BYTE_ALIGN_SIZE, buff, page_nr * NRF52_FLASH_BYTE_ALIGN_SIZE);
...
{
    int count =  nrf52_flash_erase(NRF52_FLASH_START_ADDRESS + page_addr * NRF52_FLASH_BYTE_ALIGN_SIZE, page_nr * NRF52_FLASH_BYTE_ALIGN_SIZE);

    return (count == page_nr * NRF52_FLASH_BYTE_ALIGN_SIZE) ? 0 : -1;
}

//FAL_FLASH_DEFINE nrf5_onchip_flash =
//{
//    .name = "onchip_flash",
//    .capacity = NRF52_FLASH_SIZE,
//    .block_size = NRF52_FLASH_PAGE_SIZE,
//    .page_size  = 4,
