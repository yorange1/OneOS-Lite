...
#define GPIO_POAR_INFO_MAP(__PORT__)                                                         \
    {GPIO##__PORT__, RCC_AHBENR_GPIO##__PORT__, GPIO_PortSourceGPIO##__PORT__}

#define GPIO_POAR_INFO_MAP_NODEF(__PORT__)                                                         \
{GPIO##__PORT__, RCC_AHBENR_GPIO##__PORT__, 0xFF}

static gpio_port_info_t port_info_table[] =
{
#ifdef GPIOA
    GPIO_POAR_INFO_MAP(A),
#endif
#ifdef GPIOB
    GPIO_POAR_INFO_MAP(B),
#endif
#ifdef GPIOC
    GPIO_POAR_INFO_MAP(C),
#endif
#ifdef GPIOD
    GPIO_POAR_INFO_MAP(D),
#endif
#ifdef GPIOE
    GPIO_POAR_INFO_MAP_NODEF(E),
#endif
#ifdef GPIOF
    GPIO_POAR_INFO_MAP_NODEF(F),
#endif
#ifdef GPIOG
    GPIO_POAR_INFO_MAP(G),
#endif
#ifdef GPIOH
    GPIO_POAR_INFO_MAP(H),
#endif
};
...
static void mm32_pin_init(void)
{
    ...
    mm32_gpio_info.port_info_tab_size = ITEM_NUM(port_info_table);
    mm32_gpio_info.irq_tab_size = ITEM_NUM(pin_irq_table);
    mm32_gpio_info.irq_hdr_tab_size = ITEM_NUM(irq_hdr_table);
...
static os_err_t MM32_pin_irq_enable(struct os_device *device, os_base_t pin, os_uint32_t enabled)
{
    ...
    {
        level = os_irq_lock();
        GPIO_InitStructure.GPIO_Pin   = irq_table->pinbit;
        GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_IPU;
        GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
        GPIO_Init(info->port, &GPIO_InitStructure);

        NVIC_InitStructure.NVIC_IRQChannel = irq_table->irqno;
        NVIC_InitStructure.NVIC_IRQChannelPriority = 0;
        NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
        NVIC_Init(&NVIC_InitStructure);
        
        mm32_gpio_info.exti_rcc_clkcmd(mm32_gpio_info.exit_periph, ENABLE);
        if (irq_table->pin_source != 0xFF)
            mm32_gpio_info.exti_lineconfig(info->port_source, irq_table->pin_source);
        else
            LOG_W(DBG_TAG, "pin %d not support exit!", pin);
