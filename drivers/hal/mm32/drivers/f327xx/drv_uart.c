...
void HAL_UART_IRQHandler(UART_TypeDef *huart)
{
    ...
    os_list_for_each_entry(uart, &mm32_uart_list, struct mm32_uart, list)
    {
        ...
        if ((RESET != UART_GetITStatus(huart, UART_ISR_RX))
            && ((uart->uart_info->huart->IER & UART_IER_RX) != RESET))
        {
            *(uart->rx_info.buffer + uart->rx_info.count) = UART_ReceiveData(uart->uart_info->huart);
            UART_ClearITPendingBit(huart, UART_ISR_RX);
            uart->rx_info.count++;
            if(uart->rx_info.count == uart->rx_info.size)
            {
                UART_ITConfig(uart->uart_info->huart, UART_IER_RX, DISABLE);
                UART_ITConfig(uart->uart_info->huart, UART_ISR_RXIDLE, DISABLE);
                os_hw_serial_isr_rxdone((struct os_serial_device *)uart, uart->rx_info.count);
                uart->rx_info.status = RESET;
            }
        }

        if ((RESET != UART_GetITStatus(huart, UART_ISR_RXIDLE))
            && ((uart->uart_info->huart->IER & UART_IER_RXIDLE) != RESET))
        {
            UART_ReceiveData(uart->uart_info->huart);
            UART_ClearITPendingBit(huart, UART_ISR_RXIDLE);
        ...
        if ((RESET != UART_GetITStatus(huart, UART_ISR_TX))
            && ((uart->uart_info->huart->IER & UART_IER_TX) != RESET))
        {
            UART_ClearITPendingBit(huart, UART_ISR_TX);
            uart->tx_info.count++;
            if(uart->tx_info.count == uart->tx_info.size)
            {
                UART_ITConfig(uart->uart_info->huart, UART_IER_TX, DISABLE);
            ...
            {
                UART_SendData(uart->uart_info->huart, uart->tx_info.buffer[uart->tx_info.count]);
            }
        }
        os_irq_unlock(level);
    }
...
static void mm32_uart_init(mm32_uart_info_t *uart_info)
{
    ...
    if ((uart_info->tx_pin_port == GPIOE) || (uart_info->tx_pin_port == GPIOE))
        RCC_AHBPeriphClockCmd(RCC_AHBENR_GPIOE, ENABLE);
    if ((uart_info->tx_pin_port == GPIOF) || (uart_info->tx_pin_port == GPIOF))
        RCC_AHBPeriphClockCmd(RCC_AHBENR_GPIOF, ENABLE);
    if ((uart_info->tx_pin_port == GPIOG) || (uart_info->tx_pin_port == GPIOG))
        RCC_AHBPeriphClockCmd(RCC_AHBENR_GPIOG, ENABLE);
    if ((uart_info->tx_pin_port == GPIOH) || (uart_info->tx_pin_port == GPIOH))
        RCC_AHBPeriphClockCmd(RCC_AHBENR_GPIOH, ENABLE);
    
    if ((uart_info->huart == UART1) || (uart_info->huart == UART6))
...
void mm32_uart_dma_transfer(mm32_uart_info_t *uart_info, DMA_InitTypeDef *dma_info)
{
    ...
    else if (dma_info->DMA_DIR == DMA_DIR_PeripheralSRC)
    {
        ...
        UART_ITConfig(uart_info->huart, UART_ISR_RXIDLE, ENABLE);
    }
}
...
static int mm32_uart_start_send(struct os_serial_device *serial, const os_uint8_t *buff, os_size_t size)
{
    ...
    {
        UART_ITConfig(uart->uart_info->huart, UART_IT_TXIEN, ENABLE);
        UART_SendData(uart->uart_info->huart, uart->tx_info.buffer[0]);
    }
    return size;
}
...
static int mm32_uart_stop_send(struct os_serial_device *serial)
{
    ...
    {
        UART_ITConfig(uart->uart_info->huart, UART_IT_TXIEN, DISABLE);
    }
...
static int mm32_uart_start_recv(struct os_serial_device *serial, os_uint8_t *buff, os_size_t size)
{
    ...
    {
        UART_ITConfig(uart->uart_info->huart, UART_IT_RXIEN, ENABLE);
        UART_ITConfig(uart->uart_info->huart, UART_IER_RXIDLE, ENABLE);
    }
