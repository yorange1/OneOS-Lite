...
os_err_t os_hw_spi_device_attach(const char *bus_name, const char *device_name, os_base_t pin)
{
    ...
    struct am_spi_cs *cs_pin = (struct am_spi_cs *)os_calloc(1, sizeof(struct am_spi_cs));
...
static os_uint32_t xfer(struct os_spi_device *device, struct os_spi_message *message)
{
    ...
    if (message->cs_take)
    {
        am_hal_gpio_out_bit_clear(am_spi_cs->pin);
    }
    ...
    {
        am_hal_gpio_out_bit_set(am_spi_cs->pin);
    }
